...
export class LocalRegex extends RegexEngine {
  ...
  public isLocalEngine() { return true; };

  public isRemoteEngine() { return false; };
   ...
   */
  public test(regexStr: string, modifiersStr: string, delimiters?: IDelimPair) : Promise<boolean> {
    try {
      const tmp = this._getRegexObject(regexStr, modifiersStr);
    } catch(e) {
   ...
   */
  public getLastError(id: number) : Promise<IRegexError> {
   ...
   */
  public match(inputs: string[], regexPairs: IRegexPair[], chainRegexes: boolean | undefined) : Promise<ICumulativeTestResults[]> {
    const constructedRegexes = this._getConstructedRegexes(regexPairs);
    ...
    /**
     * outerMap() applies all regexes to a given string in the input array
     * @param str
     * @param i
     */
    ...
    return Promise.resolve(inputs.map(outerMap));
  }
